function search(nums, target) {
    var left = 0; // initialize left pointer to 0
    var right = nums.length - 1; // initialize right pointer to the last index of the array
    while (left <= right) {
        // continue the loop till left pointer is less than or equal to right pointer
        var mid = left + Math.floor((right - left) / 2); // calculate the middle index of the array
        if (nums[mid] === target) {
